 ...
 * @since_tizen 3.0
 */
typedef struct _mtp_raw_device *mtp_raw_device_h;
 ...
 * @see mtp_initialize()
 */
MTP_API int mtp_get_mtp_device(int bus_location, int device_number, mtp_device_h *mtp_device);

/**
 * @brief Gets storage ids from device
 * @since_tizen 3.0
 * @remarks The @a storage_id should be freed using free().
 *
 * @return 0 on success, otherwise a negative error value.
 * @retval #MTP_ERROR_NONE Successful
 * @retval #MTP_ERROR_NOT_SUPPORTED MTP is not supported
 ...
 * @see mtp_get_object_handles()
 */
MTP_API int mtp_objectinfo_get_parent(mtp_device_h mtp_device, mtp_object_h object_handle, mtp_object_h *parent);
 ...
 * @see mtp_get_object_handles()
 */
MTP_API int mtp_objectinfo_get_storage_id(mtp_device_h mtp_device, mtp_object_h object_handle, mtp_storage_h* mtp_storage);
