...
int manager_test_get_object_handles(void)
{
	...
	for (i = 0; i < folder_max; i++) {
		...
		ret = mtp_objectinfo_get_format(mtp_device, folder_list[i], &format);
	...
	for (i = 0; i < file_max; i++) {
		...
		ret = mtp_objectinfo_get_format(mtp_device, file_list[i], &format);
		TC_PRT("ret[%d]: object id[%d] - filename[%s], format[%s]",
	...
	for (i = 0; i < list_max; i++) {
		...
		ret = mtp_objectinfo_get_format(mtp_device, mtp_objects[i], &format);
		TC_PRT("ret[%d]: object id[%d] - filename[%s], format[%s]",
			ret, mtp_objects[i], filename, test_filetype_to_string(format));
	}
...
int objectinfo_test_get_storage_id(void)
{
	...
	for (i = 0; i < list_max; i++) {
		...
		TC_PRT("ret[%d]: object id[%d] storage_id[%d]", ret, mtp_objects[i], value);
	}
...
int objectinfo_test_get_format(void)
{
	...
	for (i = 0; i < list_max; i++) {
		ret = mtp_objectinfo_get_format(mtp_device, mtp_objects[i], &value);
		TC_PRT("ret[%d]: object id[%d] format[%d]", ret, mtp_objects[i], value);
	}
...
int objectinfo_test_get_image_pix_depth(void)
{
	...
	for (i = 0; i < list_max; i++) {
		ret = mtp_objectinfo_get_image_pix_depth(mtp_device, mtp_objects[i], &value);
		TC_PRT("ret[%d]: object id[%d] image_pix_depth[%d]", ret, mtp_objects[i], value);
	}
...
int objectinfo_test_get_thumbnail_format(void)
{
	...
	for (i = 0; i < list_max; i++) {
		ret = mtp_objectinfo_get_thumbnail_format(mtp_device, mtp_objects[i], &value);
...
int application_test_get_image_from_DCIM(void)
{
	...
	while (true) {
		...
		for (i = 0; i < object_count; i++) {
			...
			ret = mtp_objectinfo_get_format(mtp_device, object_list[i], &format);
		...
		ret = mtp_objectinfo_get_format(mtp_device, selected_id, &format);
	...
	for (i = 0; i < object_count2; i++) {
		...
		ret = mtp_objectinfo_get_format(mtp_device, object_list2[i], &format);
	...
	while (true) {
		...
		ret = mtp_objectinfo_get_format(mtp_device, input_object_id2, &format);
